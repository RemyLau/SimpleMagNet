...
def generate_dataset_2class(edge_index, splits=10, test_prob=0.6):
    ...
    for i in range(splits):
        ...
        train_neg, val_neg, test_neg = split_negative(
            edge_index,
            seed=i * 10,
            prob=test_prob,
        )
...
def generate_dataset_3class(
    ...
    def iteration(ind):
        ...
        if noisy:
            ids_test, labels_test = noisy_undirected_label2directed_label(
                A,
                ids_test,
                task,
            )
        ...
        if noisy:
            ids_val, labels_val = noisy_undirected_label2directed_label(
                A,
                ids_val,
                task,
            )
        ...
        if noisy:
            ids_train, labels_train = noisy_undirected_label2directed_label(
                A,
                ids_train,
                task,
            )
...
def get_appr_directed_adj(alpha, edge_index, num_nodes, dtype, edge_weight=None):
    ...
    # L_appr
    L = (
        torch.mm(torch.mm(pi_sqrt, p_ppr), pi_inv_sqrt)
        + torch.mm(torch.mm(pi_inv_sqrt, p_ppr.t()), pi_sqrt)
    ) / 2.0
